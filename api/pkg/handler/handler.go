...
// REST API
func (h *Handler) RestHandler(w http.ResponseWriter, r *http.Request) {
...
func (h *Handler) GrammerHandler(w http.ResponseWriter, r *http.Request) {
	// ポインタ
	p := 99          //　初期値
	pt := &p         //　ポインタ作成
	...
	for i := 0; i < 100; i++ {
		...
		num.Str = append(num.Str, strconv.Itoa(i))
	}
	// json1 := `{
	// 	"name": "test"
	// }`
...
func (h *Handler) TemplateHandler(w http.ResponseWriter, r *http.Request) {
	t, _ := template.ParseFiles("./pkg/templates/tmpl.html", "./pkg/template/tmpl2.html")
	DaysOfWeek := []string{"日", "月", "火", "水", "木", "金", "土"}
	t.Execute(w, DaysOfWeek)
}

func (h *Handler) UploadHandler(w http.ResponseWriter, r *http.Request) {
	t, _ := template.ParseFiles("./pkg/templates/upload.html")
	t.Execute(w, nil)
}
